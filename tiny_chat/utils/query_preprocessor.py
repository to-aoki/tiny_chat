...
class QueryPlanner:
    ...
    def evaluate(self, question: str, query: QueryResponse, search_results=None,
        ...
        request_content = f"""# タスク"
                           "タスクは与えられた質問と検索理由（reason）に対して、有効な検索結果があるか確認し、検索結果が存在し妥当であれば質問回答に役立つ知識（knowledge）を更新することです。"
                           "質問に有効な検索結果があった場合は、その検索結果のインデックスを有効なインデックス（valid_index）として列挙してください。"
                           "また、クエリは検索結果を得るためにデータベースに発行したクエリです。"
                           "検索理由を満たした検索結果が無い、あるいは追加で検索が必要と考えた場合は、再検索フラグをtrueとして、検索理由に基づき新しくクエリを記述してください。"
                           ...
                           "new_query": "再検索が必要な場合は、検索理由から、既存のクエリと重複しない新しい検索クエリを記述。疑問点や要望などの補足説明は記載しない"" {{"}} の形式です。"
                           ""
                           "## 質問"
                           "{question}"
                           ""
